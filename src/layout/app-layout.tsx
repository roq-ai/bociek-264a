...
import { getFirmas } from 'apiSdk/firmas';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<FirmaInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/firmas` : null),
    () => getFirmas({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Firma',
      path: isTenantUser ? `/firmas/view/${data?.[0]?.id}` : '/firmas',
      entity: 'firma',
